...
from chialite.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from chialite.full_node.full_node_api import FullNodeAPI
from chialite.server.start_farmer import service_kwargs_for_farmer
from chialite.server.start_full_node import service_kwargs_for_full_node
from chialite.server.start_harvester import service_kwargs_for_harvester
from chialite.server.start_introducer import service_kwargs_for_introducer
from chialite.server.start_service import Service
from chialite.server.start_timelord import service_kwargs_for_timelord
from chialite.server.start_wallet import service_kwargs_for_wallet
from chialite.simulator.start_simulator import service_kwargs_for_full_node_simulator
from chialite.timelord.timelord_launcher import kill_processes, spawn_process
from chialite.types.peer_info import PeerInfo
from chialite.util.bech32m import encode_puzzle_hash
from chialite.util.block_tools import BlockTools, test_constants
from chialite.util.hash import std_hash
from chialite.util.ints import uint16, uint32
...
async def setup_farmer(
    ...
    config["xsh_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "xsh")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["xsh_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "xsh")
