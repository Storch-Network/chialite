...
from chialite.cmds.init_funcs import chialite_init
from chialite.daemon.windows_signal import kill
from chialite.server.server import ssl_context_for_root, ssl_context_for_server
from chialite.ssl.create_ssl import get_mozzila_ca_crt
from chialite.util.chialite_logging import initialize_logging
from chialite.util.config import load_config
from chialite.util.json_util import dict_to_json_str
from chialite.util.path import mkdir
from chialite.util.service_groups import validate_service
from chialite.util.setproctitle import setproctitle
from chialite.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Chialite.")
    quit()
...
service_plotter = "chialite plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "chialite": "chialite",
        "chialite_wallet": "start_wallet",
        "chialite_full_node": "start_full_node",
        "chialite_harvester": "start_harvester",
        "chialite_farmer": "start_farmer",
        "chialite_introducer": "start_introducer",
        "chialite_timelord": "start_timelord",
        "chialite_timelord_launcher": "timelord_launcher",
        "chialite_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("chialite daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered CHIALITE_ROOT
    os.environ["CHIALITE_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with CHIALITE_ROOT: {os.environ['CHIALITE_ROOT']}")
...
async def async_run_daemon(root_path: Path) -> int:
    chialite_init(root_path)
    config = load_config(root_path, "config.yaml")
    setproctitle("chialite_daemon")
