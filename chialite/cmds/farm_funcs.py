...
from chialite.rpc.full_node_rpc_client import FullNodeRpcClient
...
from chialite.util.misc import format_minutes
...
async def summary(rpc_port: int, wallet_rpc_port: int, harvester_rpc_port: int, farmer_rpc_port: int) -> None:
    ...
    if amounts is not None:
        print(f"Total chialite farmed: {amounts['farmed_amount'] / units['chialite']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['chialite']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['chialite']}")
        print(f"Last height farmed: {amounts['last_height_farmed']}")
    else:
        print("Total chialite farmed: Unknown")
    ...
    print("Note: log into your key using 'chialite wallet show' to see rewards for each key")
