...
from chialite.wallet.puzzles.puzzle_utils import (
...
from chialite.wallet.secret_key_store import SecretKeyStore
from chialite.wallet.sign_coin_solutions import sign_coin_solutions
...
class Wallet:
    ...
    async def create_spend_bundle_relative_chialite(self, chialite_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if chialite_amount < 0:
            utxos = await self.select_coins(abs(chialite_amount), exclude)
        else:
        ...
        chialite_amount = spend_value + chialite_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": chialite_amount}]
