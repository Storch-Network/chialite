...
from chialite.protocols import wallet_protocol
from chialite.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from chialite.protocols.protocol_message_types import ProtocolMessageTypes
from chialite.protocols.wallet_protocol import (
...
from chialite.util.errors import Err, ValidationError
from chialite.util.ints import uint32, uint128
from chialite.util.keychain import Keychain
from chialite.util.lru_cache import LRUCache
from chialite.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from chialite.wallet.settings.settings_objects import BackupInitialized
...
from chialite.wallet.wallet_blockchain import ReceiveBlockResult
from chialite.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    server: Optional[ChialiteServer]
    log: logging.Logger
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]):
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'chialite keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSChialiteConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSChialiteConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSChialiteConnection):
        if self.wallet_state_manager is None:
            return None
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSChialiteConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(self, peer: WSChialiteConnection, block_i, additions) -> Optional[List[Coin]]:
        if len(additions) > 0:
    ...
    async def get_removals(self, peer: WSChialiteConnection, block_i, additions, removals) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
